...
class jurnal extends Database
{
	...
	{
		$con = $this->dbconnect();
		$sql = "SELECT t_jurnal.id_jurnal, t_jurnal.nama, t_jurnal.hari, t_jurnal.tgl, t_jurnal.kelas, t_jurnal.jam_mulai, t_jurnal.jam_selesai, t_jurnal.kegiatan 
			FROM t_jurnal
			ORDER BY t_jurnal.id_jurnal" ;
		$query = mysqli_query($con,$sql);
		while($item = mysqli_fetch_array($query)){
			$hasil[] = $item;
		}
